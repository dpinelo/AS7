 ...
 * @param operation One of DiffCode::INSERT, DiffCode::DELETE or DiffCode::EQUAL
 * @param text The text being applied
 */
Diff::Diff(DiffCode::Operation _operation, const QString &_text) :
...
QString Diff::strOperation(DiffCode::Operation op)
{
    switch (op)
    {
    case DiffCode::DIFF_INSERT:
        return "DiffCode::INSERT";
    case DiffCode::DIFF_DELETE:
        return "DiffCode::DELETE";
    case DiffCode::DIFF_EQUAL:
        return "DiffCode::EQUAL";
    }
...
QList<Diff> diff_match_patch::diff_main_lines(const QString &text1, const QString &text2)
{
    QList<QVariant> linesResult = diff_linesToChars(text1, text2);
    QString lineText1 = linesResult.at(0).toString();
    QString lineText2 = linesResult.at(1).toString();
    QStringList lineArray = linesResult.at(2).toStringList();
    QList<Diff> diffs = diff_main(lineText1, lineText2, false);
                                        ...
                                        const QString &text2, bool checklines, clock_t deadline)
{
    ...
    if (text1 == text2)
    {
        if (!text1.isEmpty())
        {
            diffs.append(Diff(DiffCode::DIFF_EQUAL, text1));
        }
        return diffs;
    }
    ...
    {
        diffs.prepend(Diff(DiffCode::DIFF_EQUAL, commonprefix));
    }
    if (!commonsuffix.isEmpty())
    {
        diffs.append(Diff(DiffCode::DIFF_EQUAL, commonsuffix));
    }
        ...
        bool checklines, clock_t deadline)
{
    ...
    if (text1.isEmpty())
    {
        ...
        diffs.append(Diff(DiffCode::DIFF_INSERT, text2));
        return diffs;
    }
    ...
    if (text2.isEmpty())
    {
        ...
        diffs.append(Diff(DiffCode::DIFF_DELETE, text1));
        return diffs;
    }

    {
        ...
        if (i != -1)
        {
            ...
            const DiffCode::Operation op = (text1.length() > text2.length()) ? DiffCode::DIFF_DELETE : DiffCode::DIFF_INSERT;
            diffs.append(Diff(op, longtext.left(i)));
            diffs.append(Diff(DiffCode::DIFF_EQUAL, shorttext));
    ...
    if (hm.count() > 0)
    {
        ...
        // Merge the results.
        diffs = diffs_a;
        diffs.append(Diff(DiffCode::DIFF_EQUAL, mid_common));
        diffs += diffs_b;
        return diffs;
    }
        ...
        clock_t deadline)
{
    ...
    diffs.append(Diff(DiffCode::DIFF_EQUAL, ""));
    int count_delete = 0;
    int count_insert = 0;
    QString text_delete = "";
    QString text_insert = "";
...
void diff_match_patch::diff_cleanupSemantic(QList<Diff> &diffs)
{
    ...
    while (thisDiff != NULL)
    {
        if (thisDiff->operation == DiffCode::DIFF_EQUAL)
        {
            // Equality found.
        ...
        else
        {
            ...
            if (thisDiff->operation == DiffCode::DIFF_INSERT)
            {
                        ...
                        <= std::max(length_insertions2, length_deletions2)))
            {
                ...
                pointer.setValue(Diff(DiffCode::DIFF_DELETE, lastequality));
                // Insert a corresponding an insert.
                pointer.insert(Diff(DiffCode::DIFF_INSERT, lastequality));
    ...
    while (thisDiff != NULL)
    {
        if (prevDiff->operation == DiffCode::DIFF_DELETE &&
                thisDiff->operation == DiffCode::DIFF_INSERT)
        {
            ...
            if (overlap_length1 >= overlap_length2)
            {
                        ...
                        overlap_length1 >= insertion.length() / 2.0)
                {
                    ...
                    pointer.previous();
                    pointer.insert(Diff(DiffCode::DIFF_EQUAL, insertion.left(overlap_length1)));
                    prevDiff->text =
            ...
            else
            {
                        ...
                        overlap_length2 >= insertion.length() / 2.0)
                {
                    ...
                    pointer.previous();
                    pointer.insert(Diff(DiffCode::DIFF_EQUAL, deletion.left(overlap_length2)));
                    prevDiff->operation = DiffCode::DIFF_INSERT;
                    prevDiff->text =
                        insertion.left(insertion.length() - overlap_length2);
                    thisDiff->operation = DiffCode::DIFF_DELETE;
...
void diff_match_patch::diff_cleanupSemanticLossless(QList<Diff> &diffs)
{
    ...
    while (nextDiff != NULL)
    {
        if (prevDiff->operation == DiffCode::DIFF_EQUAL &&
                nextDiff->operation == DiffCode::DIFF_EQUAL)
        {
...
void diff_match_patch::diff_cleanupEfficiency(QList<Diff> &diffs)
{
    ...
    while (thisDiff != NULL)
    {
        ...
        {
            // An insertion or deletion.
            if (thisDiff->operation == DiffCode::DIFF_DELETE)
            {
                post_del = true;
            }
            else
...
{
    diffs.append(Diff(DiffCode::DIFF_EQUAL, ""));  // Add a dummy entry at the end.
    QMutableListIterator<Diff> pointer(diffs);
    int count_delete = 0;
    int count_insert = 0;
    QString text_delete = "";
            ...
            if (count_delete + count_insert > 1)
            {
                ...
                if (both_types)
                {
                    ...
                    if (commonlength != 0)
                    {
                        ...
                        {
                            pointer.insert(Diff(DiffCode::DIFF_EQUAL, text_insert.left(commonlength)));
                        }
                ...
                {
                    pointer.insert(Diff(DiffCode::DIFF_DELETE, text_delete));
                }
                if (!text_insert.isEmpty())
                {
                    pointer.insert(Diff(DiffCode::DIFF_INSERT, text_insert));
                }
...
int diff_match_patch::diff_xIndex(const QList<Diff> &diffs, int loc)
{
    ...
    foreach(Diff aDiff, diffs)
    {
        if (aDiff.operation != DiffCode::DIFF_INSERT)
        {
            // Equality or deletion.
            chars1 += aDiff.text.length();
        }
        if (aDiff.operation != DiffCode::DIFF_DELETE)
        {
    ...
    if (lastDiff.operation == DiffCode::DIFF_DELETE)
    {
        ...
        const QString &delta)
{
    ...
    foreach(QString token, tokens)
    {
        ...
        case '+':
            ...
            diffs.append(Diff(DiffCode::DIFF_INSERT, param));
            break;
        case '-':
            // Fall through.
        case '=':
        {
            ...
            if (token[0] == QChar('='))
            {
                diffs.append(Diff(DiffCode::DIFF_EQUAL, text));
            }
            else
            {
                diffs.append(Diff(DiffCode::DIFF_DELETE, text));
            }
            break;
        }
        default:
...
void diff_match_patch::patch_addContext(Patch &patch, const QString &text)
{
    ...
    if (!prefix.isEmpty())
    {
        patch.diffs.prepend(Diff(DiffCode::DIFF_EQUAL, prefix));
    }
    // Add the suffix.
    ...
    if (!suffix.isEmpty())
    {
        patch.diffs.append(Diff(DiffCode::DIFF_EQUAL, suffix));
    }
        ...
        const QList<Diff> &diffs)
{
    ...
    foreach(Diff aDiff, diffs)
    {
        if (patch.diffs.isEmpty() && aDiff.operation != DiffCode::DIFF_EQUAL)
        {
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    foreach(Diff aDiff, aPatch.diffs)
                    {
                        if (aDiff.operation != DiffCode::DIFF_EQUAL)
                        {
                            ...
                            else if (aDiff.operation == DiffCode::DIFF_DELETE)
                            {
                                // Deletion
...
QString diff_match_patch::patch_addPadding(QList<Patch> &patches)
{
    ...
    if (firstPatchDiffs.empty() || firstPatchDiffs.first().operation != DiffCode::DIFF_EQUAL)
    {
        // Add nullPadding equality.
        firstPatchDiffs.prepend(Diff(DiffCode::DIFF_EQUAL, nullPadding));
    ...
    if (lastPatchDiffs.empty() || lastPatchDiffs.last().operation != DiffCode::DIFF_EQUAL)
    {
        // Add nullPadding equality.
        lastPatchDiffs.append(Diff(DiffCode::DIFF_EQUAL, nullPadding));
    ...
    while (!bigpatch.isNull())
    {
        ...
        while (!bigpatch.diffs.isEmpty())
        {
            ...
            if (!precontext.isEmpty())
            {
                ...
                patch.diffs.append(Diff(DiffCode::DIFF_EQUAL, precontext));
            }
            while (!bigpatch.diffs.isEmpty()
                    && patch.length1 < patch_size - Patch_Margin)
            {
                ...
                if (diff_type == DiffCode::DIFF_INSERT)
                {
                ...
                else if (diff_type == DiffCode::DIFF_DELETE && patch.diffs.size() == 1
                         && patch.diffs.front().operation == DiffCode::DIFF_EQUAL
                ...
                else
                {
                    ...
                    patch.length1 += diff_text.length();
                    start1 += diff_text.length();
                    if (diff_type == DiffCode::DIFF_EQUAL)
                    {
                        patch.length2 += diff_text.length();
                        start2 += diff_text.length();
                    }
            ...
            if (!postcontext.isEmpty())
            {
                ...
                if (!patch.diffs.isEmpty()
                        && patch.diffs.back().operation == DiffCode::DIFF_EQUAL)
                {
                    patch.diffs.back().text += postcontext;
                }
                else
                {
                    patch.diffs.append(Diff(DiffCode::DIFF_EQUAL, postcontext));
                }
            }
            if (!empty)
            {
...
QList<Patch> diff_match_patch::patch_fromText(const QString &textline)
{
    ...
    while (!text.isEmpty())
    {
        ...
        while (!text.isEmpty())
        {
            ...
            {
                // Deletion.
                patch.diffs.append(Diff(DiffCode::DIFF_DELETE, line));
            }
            else if (sign == '+')
            {
                // Insertion.
                patch.diffs.append(Diff(DiffCode::DIFF_INSERT, line));
            }
            else if (sign == ' ')
            {
                // Minor equality.
                patch.diffs.append(Diff(DiffCode::DIFF_EQUAL, line));
            }
            else if (sign == '@')
            {
                // Start of next patch.
