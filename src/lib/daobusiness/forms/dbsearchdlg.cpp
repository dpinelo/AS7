...
DBSearchDlg::DBSearchDlg(const QString &tableName, bool initContext, QWidget *parent) :
    AERPBaseDialog(parent),
    ui(new Ui::DBSearchDlg),
    d(new DBSearchDlgPrivate(this))
{
    d->m_initContext = initContext;
...
BaseBeanSharedPointerList DBSearchDlg::checkedBeans()
{
    ...
    if ( d->m_model )
    {
        ...
        checkedBeans = d->m_model->beans(list);
    }
    return checkedBeans;
}
...
DBTableView *DBSearchDlg::tableView() const
{
    return ui->tvResults;
}

DBTreeView *DBSearchDlg::treeView() const
{
...
void DBSearchDlg::newRecord()
{
    ...
    CommonsFunctions::setOverrideCursor(QCursor(Qt::WaitCursor));
    QScopedPointer<DBRecordDlg> dlg (new DBRecordDlg(insertBean, openType, d->m_initContext, this));
    CommonsFunctions::restoreOverrideCursor();
    if ( dlg->openSuccess() && dlg->init() )
    {
        dlg->setModal(true);
