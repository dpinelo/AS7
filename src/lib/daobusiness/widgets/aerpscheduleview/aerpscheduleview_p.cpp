...
/*
 * El modo de funcionamiento del control se basa en calcular un offset temporal (en segundos) a partir de la ubicación
 * del item en filas y columnas. Para la visualización de un día o una semana, esta visualización hace fácil entender
 * el offset: el eje vertical marca el tiempo (horas) y el eje horizontal el día.
 * En el caso de visualización del mes, el usuario ve semanas, y las filas del Schedule View son semanas. Sin embargo
 * internamente se complica un poco el modelo, porque cada semana es como una repetición o iteración de una visualzación
 * de un día/semana. Es decir, tendremos varios ejes de coordenadas por semana
 **/
int AERPScheduleViewPrivate::offsetToVisualColumn(const int iOffset) const
{
    if ( iOffset < 0 )
    {
        return -1;
    }
    if (qxt_p().viewMode() == AERPScheduleView::DayView || qxt_p().viewMode() == AERPScheduleView::WeekView)
    {
        return iOffset / qxt_p().rows();
    }
    else if (qxt_p().viewMode() == AERPScheduleView::MonthView && qxt_p().model())
    {
        int offsetOfWeek = qxt_p().cols() * qxt_p().rowsInDayOfWeek();
        // Buscamos la ubicación en su semana actual
        int previousWeeks = iOffset / offsetOfWeek;
        int offsetInCurrentRow = iOffset - (previousWeeks * offsetOfWeek);
        return (int)(offsetInCurrentRow / qxt_p().rowsInDayOfWeek());
    }
    return -1;
}

int AERPScheduleViewPrivate::offsetToVisualRow(const int iOffset) const
{
    if (qxt_p().viewMode() == AERPScheduleView::DayView || qxt_p().viewMode() == AERPScheduleView::WeekView)
    {
    ...
    else if (qxt_p().viewMode() == AERPScheduleView::MonthView && qxt_p().model())
    {
        ...
        // Buscamos la ubicación en su semana actual
        int previousWeeks = iOffset / offsetOfWeek;
        int previousWeekOffset = previousWeeks * qxt_p().rowsInDayOfWeek() * qxt_p().cols();
        int visualCol = offsetToVisualColumn(iOffset);
        int relativeOffsetInThisWeek = iOffset - previousWeekOffset - (visualCol * qxt_p().rowsInDayOfWeek());
        return (previousWeeks * qxt_p().rowsInDayOfWeek()) + relativeOffsetInThisWeek;
    }
    return -1;
}

int AERPScheduleViewPrivate::visualIndexToOffset(const int iRow, const int iCol) const
{
    if (qxt_p().viewMode() == AERPScheduleView::DayView || qxt_p().viewMode() == AERPScheduleView::WeekView)
    {
    ...
    {
        int previousWeekCount = weekIndexForRow(iRow);
        int offsetOfPreviousWeek = previousWeekCount * qxt_p().rowsInDayOfWeek() * qxt_p().cols();
        int offsetOfCurrentWeek = iCol * qxt_p().rowsInDayOfWeek();
        int offsetRowTranslate = iRow - (previousWeekCount * qxt_p().rowsInDayOfWeek());
        return offsetOfCurrentWeek + offsetOfPreviousWeek + offsetRowTranslate;
    }
    return -1;
}

QVector< QRect > AERPScheduleViewPrivate::calculateRangeGeometries(const int iStartOffset, const int iEndOffset) const
{
    QVector<QRect> rects;
...
int AERPScheduleViewPrivate::pointToOffset(const QPoint & point)
{
...
void AERPScheduleViewPrivate::reloadItemsFromModel()
{
    qDeleteAll(m_Items.begin(), m_Items.end());
    m_Items.clear();
    if ( qxt_p().selectionModel() )
    {
        qxt_p().selectionModel()->clear();
    }
...
void AERPScheduleViewPrivate::init()
{
    if (qxt_p().model())
    {
        ...
        if (!m_vHeader)
        {
            m_vHeader = new AERPScheduleHeaderWidget(Qt::Vertical, &qxt_p());
        ...
        if (!m_hHeader)
        {
            m_hHeader = new AERPScheduleHeaderWidget(Qt::Horizontal, &qxt_p());
            connect(m_hHeader, SIGNAL(geometriesChanged()), &qxt_p(), SLOT(updateGeometries()));
        }
        m_hHeader->show();

        /*here we also initialize the items*/
        m_vHeader->setDefaultSectionSize(m_defaultRowSize);
 ...
 */
QList< QLinkedList<AERPScheduleInternalItem *> > AERPScheduleViewPrivate::findConcurrentItems(const int from, const int to) const
{
    QList< QLinkedList<AERPScheduleInternalItem *> > allConcurrentItems;

    QList<AERPScheduleInternalItem *> allItemsSorted = m_Items;
    ...
    QLinkedList<AERPScheduleInternalItem *> concurrentItems;
        ...
        if (tempEndOffset >= startOffset && tempStartOffset <= endOffset)
        {
            ...
            if (concurrentItems.size() >= 1)
            {
                ...
                */
                ...
                for (QLinkedList<AERPScheduleInternalItem *>::iterator it = concurrentItems.begin(); it != concurrentItems.end(); ++it)
                {
...
/**
 * @brief AERPScheduleViewPrivate::weekNumberForRow
 * Devuelve el número de semana del año para la fila lógica indicada (en el método mes, la fila, indica una hora)
 * @param iRow
 * @return
 */
int AERPScheduleViewPrivate::weekNumberForRow(const int iRow) const
{
    QDateTime startTime = QDateTime::fromTime_t(m_startUnixTime);
    int weeksOnView = qxt_p().weeksOnRange();
    int rowCount = qxt_p().rows();
    int rowPerWeek = rowCount / weeksOnView;
    return (int) (iRow / rowPerWeek) + startTime.date().weekNumber();
}

/**
 * @brief AERPScheduleViewPrivate::weekIndexForRow
 * Devuelve la fila que representa una semana en la que está contenida la hora iRow
 * @param iRow
 * @return
 */
int AERPScheduleViewPrivate::weekIndexForRow(const int iRow) const
{
    int weeksOnView = qxt_p().weeksOnRange();
    int rowCount = qxt_p().rows();
    int rowPerWeek = rowCount / weeksOnView;
    return (int) (iRow / rowPerWeek);
}

void AERPScheduleViewPrivate::setDefaultRowSize(int rowSize)
{
    m_defaultRowSize = rowSize;
    if ( m_vHeader )
    {
        m_vHeader->setDefaultSectionSize(rowSize);
    }
}

void AERPScheduleViewPrivate::handleItemConcurrency(const int from, const int to)
{
    ...
    QList< QLinkedList<AERPScheduleInternalItem *> > allConcurrentItems =  findConcurrentItems(from, to);
    ...
    QList< QList< AERPScheduleInternalItem *> >virtualTable;

    for (int iListLoop = 0; iListLoop < allConcurrentItems.size(); iListLoop++)
    {
        QLinkedList<AERPScheduleInternalItem *> & currentItems = allConcurrentItems[iListLoop];
        QList< AERPScheduleInternalItem * > currentColumn;
        ...
        {
            QMutableLinkedListIterator< AERPScheduleInternalItem * > iter(currentItems);
...
AERPScheduleViewPrivate::AERPScheduleViewPrivate()
{
    m_Cols = 0;
    m_vHeader = 0;
    m_hHeader = 0;
    ...
    m_readOnly = true;
    m_defaultRowSize = 20;
    m_resizeRowsToFitView = false;
    m_htmlTodayCellColor = "#acbed0";

    connect(&scrollTimer, SIGNAL(timeout()), this, SLOT(scrollTimerTimeout()));
}

void AERPScheduleViewPrivate::itemGeometryChanged(AERPScheduleInternalItem * item, QVector< QRect > oldGeometry)
{
    QRegion oldRegion;
...
int AERPScheduleViewPrivate::unixTimeToOffset(const uint constUnixTime, bool indexEndTime) const
{
    ...
    if (unixTime >= m_startUnixTime && unixTime <= m_endUnixTime)
    {
        ...
        else if ( qxt_p().viewMode() == AERPScheduleView::MonthView )
        {
            qint32 iCol = offsetToVisualColumn(iOffset);
            qint32 iRow = offsetToVisualRow(iOffset);
            return visualIndexToOffset(iRow, iCol);
        }
    }
...
void AERPScheduleViewPrivate::scrollTimerTimeout()
{
...
int AERPScheduleViewPrivate::offsetToUnixTime(const int offset, bool indexEndTime) const
{
    qint32 rows = qxt_p().rows();
    uint unixTime = 0;
    if ( qxt_p().viewMode() == AERPScheduleView::DayView || qxt_p().viewMode() == AERPScheduleView::WeekView )
    {
        int iRow = offsetToVisualRow(offset);
        int iCol = offsetToVisualColumn(offset);
        unixTime = (iRow + (iCol * rows)) * m_currentZoomDepth;
    }
    else if ( qxt_p().viewMode() == AERPScheduleView::MonthView )
    {
        unixTime = offset * m_currentZoomDepth;
    }
    else
    {
        return -1;
...
AERPScheduleInternalItem::AERPScheduleInternalItem(AERPScheduleView *parent, QModelIndex index, QVector<QRect> geometries)
...
{
    return qobject_cast<AERPScheduleView *>(parent());
}

/*!
 * @desc returns the currently VISUAL offset (changes when a item moves)
 */
int AERPScheduleInternalItem::visualStartTableOffset() const
{
 ...
 * @desc returns the currently VISUAL offset (changes when a item moves)
 */
int AERPScheduleInternalItem::visualEndTableOffset() const
{
    if (m_geometries.size() == 0 || !parentView())
    {
        return -1;
...
bool AERPScheduleInternalItem::setData(QVariant data, int role)
{
...
QVariant AERPScheduleInternalItem::data(int role) const
{
...
void AERPScheduleInternalItem::setStartTableOffset(int iOffset)
{
    if (parentView() && parentView()->model())
    {
...
void AERPScheduleInternalItem::setRowsUsed(int rows)
{
    if (parentView() && parentView()->model())
    {
...
bool AERPScheduleInternalItem::contains(const QPoint & pt)
{
...
void AERPScheduleInternalItem::setGeometry(const QVector< QRect > geo)
{
    if (!this->parent())
    {
        return;
...
QVector< QRect > AERPScheduleInternalItem::geometry() const
{
    return this->m_geometries;
}

void AERPScheduleInternalItem::startMove()
{
...
void AERPScheduleInternalItem::resetMove()
{
...
void AERPScheduleInternalItem::stopMove()
{
    this->m_SavedGeometries.clear();
    m_moving = false;
}

QModelIndex AERPScheduleInternalItem::modelIndex() const
{
    QModelIndex indx;
    if (parentView() && parentView()->model())
    {
