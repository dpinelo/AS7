...
#define LIGHTER_FACTOR 180

AERPScheduleItemDelegate::AERPScheduleItemDelegate(QObject *parent)
    : QAbstractItemDelegate(parent)
{
}


AERPScheduleItemDelegate::~AERPScheduleItemDelegate()
{
}
...
void AERPScheduleItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    for (int iLoop = 0; iLoop < agendaOption->itemGeometries.size(); iLoop++)
    {
                ...
                || (*agendaOption->itemPaintCache)[iLoop].height() != agendaOption->itemGeometries[iLoop].height())
        {
            ...
            // Dibujamos primero una sombra muy mona
            //paintShadow(&cachePainter, rect, agendaOption, index);
        ...
        // Dibujamos primero una sombra muy mona
        //paintShadow(painter, currRect, agendaOption, index);
 ...
 */
void AERPScheduleItemDelegate::paintItemBody(QPainter *painter, const QRect rect, const AERPStyleOptionScheduleViewItem & option, const ItemPart part, const QModelIndex & index) const
{
    int iCurrRoundTop, iCurrRoundBottom;
    iCurrRoundTop = iCurrRoundBottom = 0;

    QColor endFillColor = index.data(Qt::BackgroundRole).value<QColor>();
    if ( !endFillColor.isValid() )
    {
        endFillColor = QColor(Qt::darkRed);
    }
    QColor startFillColor = endFillColor.lighter(LIGHTER_FACTOR);
    QColor outLineColor;
    if ( option.selectedItem )
    {
        outLineColor = CommonsFunctions::contrastedColor(Qt::white, endFillColor);
    }
    else
    {
        outLineColor = endFillColor.darker();
    }
    ...
    gradient.setStart(cacheRect.topLeft());
    gradient.setFinalStop(cacheRect.bottomLeft());
    gradient.setColorAt((qreal)0, startFillColor);
    gradient.setColorAt((qreal)1, endFillColor);
    ...
    // Escribimos ahora el texto
    QString text = index.data(AlephERP::BaseBeanStringRole).toString();
    ...
    if ( !text.isEmpty() )
    {
        ...
        QColor fontColor;
        int middleGray = qGray(endFillColor.rgb());
        if ( middleGray > 186 )
        {
            fontColor = Qt::black;
        }
 ...
 * \a const QModelIndex &index the items model index
 */
void AERPScheduleItemDelegate::paintItemHeader(QPainter *painter, const QRect rect , const AERPStyleOptionScheduleViewItem & option, const QModelIndex &index) const
{
    ...
    {
        fillColor = QColor(Qt::darkRed);
    }
    painter->setPen(fillColor.lighter(LIGHTER_FACTOR).lighter(LIGHTER_FACTOR));
    // Para decorar algo más, ponemos una línea marcando la cabecera.
    ...
    QString text = index.data(AlephERP::ScheduleTitleRole).toString();
    if ( text.isEmpty() )
    {
        text = startTime.toString("hh:mm") + ' ' + endTime.toString("hh:mm");
    }

    QFontMetrics metr(font);
    QColor fontColor;
    int middleGray = qGray(fillColor.rgb());
    if ( middleGray > 186 )
    {
        fontColor = Qt::black;
    }
 ...
 * \a const QModelIndex &index the items model index
 */
void AERPScheduleItemDelegate::paintSubItem(QPainter * /*painter*/, const QRect /*rect*/, const AERPStyleOptionScheduleViewItem & /*option*/, const QModelIndex & /*index*/) const
{
 ...
 */
QSize AERPScheduleItemDelegate::sizeHint(const QStyleOptionViewItem & option, const QModelIndex & index) const
{
...
void AERPScheduleItemDelegate::createPainterPath(QPainterPath & emptyPath, const QRect & fullItemRect, const int iRoundTop, const int iRoundBottom) const
{
...
void AERPScheduleItemDelegate::paintShadow(QPainter *painter, const QRect &rect, const AERPStyleOptionScheduleViewItem *option, const QModelIndex &index) const
{
    // Dibujamos primero una sombra muy mona
    // Será un rectángulo situado debajo del item en sí
    int shadowThickness = 2;

    painter->save();

    QRect shadowRect = QRect(rect.topLeft() + QPoint(shadowThickness, shadowThickness), rect.bottomRight() + QPoint(shadowThickness, shadowThickness));
    QColor initColor = index.data(Qt::BackgroundRole).value<QColor>();
    initColor.setAlpha(60);

    QBrush brush(initColor);

    // Para el efecto deseado, no necesitamos que se dibuje el borde. Se elimina.
    QPen outerPen;
    outerPen.setStyle(Qt::NoPen);
    painter->setPen(outerPen);

#if (QT_VERSION < QT_VERSION_CHECK(5,0,0))
    painter->setBrush(brush);
    painter->drawRoundRect(shadowRect, option->roundCornersRadius, option->roundCornersRadius);
#else
    painter->setBrush(brush);
    painter->drawRoundedRect(shadowRect, option->roundCornersRadius, option->roundCornersRadius);
#endif
