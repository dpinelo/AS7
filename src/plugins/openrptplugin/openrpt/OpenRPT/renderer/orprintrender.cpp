  ...
  for(int i = 0; i < p->primitives(); i++)
  {
    ...
    if(prim->type() == OROTextBox::TextBox)
    {
      ...
      else
      {
        ...
        if ( tb->html() )
        {
            // Vamos a obtener el tamaño ideal
            QTextDocument docCalcSize;
            docCalcSize.setTextWidth(sz.width());
            docCalcSize.setHtml(text);
            rc = QRectF(0, 0, sz.width() * xDpi, docCalcSize.size().height() * yDpi);
