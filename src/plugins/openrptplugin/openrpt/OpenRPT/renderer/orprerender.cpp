...
class ORPreRenderPrivate {
  public:
    ...
    // dpinelo: Modificado para soportar HTML
    void addTextPrimitive(ORObject *element, QPointF pos, QSizeF size, int align, QString text, QFont font = QFont(), bool html = false);
    QString evaluateField(ORFieldData* f);
    qreal renderSectionSize(const ORSectionData &, bool = false);

    // dpinelo -------------------------------------
    QString formatUsingAlephERP(ORFieldData *f, const QVariant &value);
    // dpinelo -------------------------------------
...
void ORPreRenderPrivate::renderBackground(OROPage * p)
{
  ...
  if(_reportData != 0 && doBg) {
    ...
    if(dynamicBg) {
      ...
      // dpinelo: -------------
      QByteArray imgdata = QByteArray::fromBase64(uudata.toUtf8());
      // dpinelo: -------------
  ...
  if(detailData.detail != 0)
  {
    ...
    if (orqThis->getQuery() && ((query = orqThis->getQuery())->size()))
    {
      ...
      do
      {
        ...
        if ( renderSectionSize(*(detailData.detail), false) + finishCurPageSize((l+1 == query->size())) + _bottomMargin + _yOffset >= _maxHeight)
        // dpinelo: --------------
        {
          if(l > 0)
            query->prev();
...
qreal ORPreRenderPrivate::renderSectionSize(const ORSectionData & sectionData, bool ignoreTextArea)
{
  ...
  for(int it = 0; it < sectionData.objects.size(); ++it)
  {
    ...
    if (element->isField())
    {
      ...
      if (fldData->align & Qt::TextWordWrap)
      {
        QString textToShow = evaluateField(fldData);
        qreal fieldHeight = (fldData->rect.y() + fldData->calculateHeightWithWordWrap(textToShow)) / 100.0;
        if (sectionHeight < fieldHeight )
          sectionHeight = fieldHeight;
      }
    }
    // dpinelo: ---------------------------
  }
...
#else
      addTextPrimitive(elemThis, pos, size, l->align, qApp->translate(_reportData->name.toUtf8().data(), l->string.toUtf8().data(), 0, 0), l->font);
#endif
    }
          ...
          if (xqry->isValid() || (nbOfCol == 1 && nbOfLines == 1))
                {
                    ...
                    if ( f->alephERPFormat )
                    {
                        QStringList parts = f->alephERPFormatString.split(".");
                        if ( parts.size() == 2 )
                        {
                            BaseBeanMetadata *bean = BeansFactory::metadataBean(parts.at(0));
                            if ( bean != NULL )
                            {
                    ...
                    addTextPrimitive(elemThis, QPointF(x, y), size, f->align, evaluateField(elemThis->toField()), f->font, html);
                    // dpinelo ----------------------------------------
                }
          else
            break;
...
// the graph drawing code.
  ...
  // dpinelo: ----------------------
  // Si el field tiene un height mayor que el tamaño de la sección (porque tiene un wordwrap activado), hay que incrementar el offset de la sección
  qreal realSectionHeight = renderSectionSize(sectionData);
  if ( realSectionHeight > intHeight )
  {
      intHeight = realSectionHeight;
  }
  // dpinelo: ----------------------
...
void ORPreRenderPrivate::addTextPrimitive(ORObject *element, QPointF pos, QSizeF size, int align, QString text, QFont font, bool html)
{
...
QString ORPreRenderPrivate::formatUsingAlephERP(ORFieldData *f, const QVariant &value)
{
    QString str;
    QStringList parts = f->alephERPFormatString.split(".");
    if ( parts.size() == 2 )
    {
        ...
        if ( bean != NULL )
        {
            ...
            if ( field != NULL )
            {
                str = field->displayValue(value, NULL);
            }
            else
            {
                str = value.toString();
...
QString ORPreRenderPrivate::evaluateField(ORFieldData* f)
{
    ...
    else
    {
        ...
        // dpinelo -----------------------------------------------------
        // dpinelo: Voy a añadir algo más, que esto está muy parco
        else if(f->data.query == "Context Query" && f->data.column == "now")
        {
            dataValue = QDateTime::currentDateTime();
            str = alephERPSettings->locale()->toString(QDateTime::currentDateTime(), CommonsFunctions::dateFormat());
        }
        else if (f->data.query == "Parameter Query")
        {
            dataValue = _lstParameters.value(f->data.column);
            str = dataValue.toString();
            d_val = dataValue.toDouble(&isFloat);
        }
        // dpinelo -----------------------------------------------------
        else
        {
            ...
            dataValue = dataThis.getVariant();
        }
    }

    // dpinelo : ---------------------------------------------------
    // dpinelo: El código de formateo lo voy a cambiar un poco...
    if(f->alephERPFormat)
    {
        str = formatUsingAlephERP(f, dataValue);
    }
    else
    // dpinelo : ---------------------------------------------------
        // formatting
    ...
    if(parseReport(_internal->_docReport.documentElement(),*(_internal->_reportData), database()))
    {
      ...
      if(_internal->_reportData->bgData.enabled)
      {
        if(_internal->_reportData->bgData.staticImage)
        {
          ...
          {
            // dpinelo: ------------
            QByteArray imgdata = QByteArray::fromBase64(_internal->_reportData->bgData.image.toUtf8());
            // dpinelo: ------------
