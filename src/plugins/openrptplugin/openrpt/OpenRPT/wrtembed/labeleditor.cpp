    ...
    : QDialog(parent, fl)
{
    ...
    // dpinelo
    connect(tbText, SIGNAL(textChanged()), this, SLOT(tbText_textChanged()));
    connect(rbHAlignJustify, SIGNAL(clicked()), this, SLOT(rbAlign_changed()));
...
void LabelEditor::rbAlign_changed()
{
    ...
    // dpinelo
    if(rbHAlignJustify->isChecked()) f |= Qt::AlignJustify;
...
// dpinelo
void LabelEditor::tbText_textChanged()
{
    // ok update the preview
    labelPreview->setText(tbText->toPlainText());
}
...
void LabelEditor::setLabelFlags( int f )
{
    ...
    // dpinelo
    } else if((f & Qt::AlignJustify) == Qt::AlignJustify) {
        rbHAlignJustify->setChecked(true);
    } else {
        //qDebug("HAlignNone");
    ...
    // dpinelo
    labelPreview->setWordWrap((f & Qt::TextWordWrap) == Qt::TextWordWrap);
    _cbWordWrap->setChecked((f & Qt::TextWordWrap) == Qt::TextWordWrap);
}
