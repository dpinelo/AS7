  ...
  : QGraphicsRectItem(0, 0, 100, 25, parent), _rotation(0), _border(QPen(Qt::black, 0)), _moved(false)
{
  ...
  if(!isSelected())
  {
    ...
    // Qt5 aplica por defecto un width de 1px y Qt4 un width de 0.
    _border.setWidth(0);
}
...
void ORGraphicsRectItem::buildXMLCommon(QDomDocument & doc, QDomElement & entity)
{
    ...
    // dpinelo: Para evitar problemas con Qt5 (donde QPen tiene como width=1 en lugar de 0, como en Qt4, eliminamos este if
    //if(pen().width() != 0) {
    ...
    // dpinelo: Para evitar problemas con Qt5 (donde QPen tiene como width=1 en lugar de 0, como en Qt4, eliminamos este if
    //if(_border.width() != 0) {
    QDomElement bwidth = doc.createElement("borderwidth");
    bwidth.appendChild(doc.createTextNode(QString::number(_border.width())));
    element.appendChild(bwidth);
    //}
...
void ORGraphicsLabelItem::buildXML(QDomDocument & doc, QDomElement & parent)
{
  ...
  // dpinelo --------------------------------------------------
    ...
    entity.appendChild(doc.createElement("justify"));
  // dpinelo --------------------------------------------------
  else // Qt::AlignLeft
  ...
  : ORGraphicsRectItem(parent)
{
  ...
  for(int i = 0; i < nl.count(); i++)
  {
    ...
    // dpinelo -------------------
      ...
      _flags |= Qt::AlignJustify;
    }  else if(n == "alephERPFormat") {
      _alephERPFormat = true;
      _alephERPFormatString = node.firstChild().nodeValue();
    // dpinelo. ----------------
    } else if(n == "top") {
      _flags |= Qt::AlignTop;
    } else if(n == "vcenter") {
...
void ORGraphicsFieldItem::buildXML(QDomDocument & doc, QDomElement & parent)
{
  ...
  // dpinelo: Formato AlephERP ---------------------
  if(_alephERPFormat)
  {
    QDomElement alephERPFormatElement = doc.createElement("alephERPFormat");
    alephERPFormatElement.appendChild(doc.createTextNode(_alephERPFormatString));
    entity.appendChild(alephERPFormatElement);
  }
  // dpinelo ---------------------------
...
void ORGraphicsFieldItem::properties(QWidget * parent)
{
  ...
  // dpinelo: ---------------------------------------------
  } else if (_alephERPFormat) {
    le->_rbAlephERP->setChecked(true);
    le->_leAlephERPFormat->setText(_alephERPFormatString);
  // dpinelo: ---------------------------------------------
  } else {
  ...
  if(le->exec() == QDialog::Accepted)
  {
    ...
    // dpinelo --------------------------------------------------------------------------
    if(le->_rbAlephERP->isChecked())
    {
      setAlephERPFormat(le->_leAlephERPFormat->text());
    }
    if(trackTotal()) {
      setUseSubTotal(le->_cbSubTotal->isChecked());
      _alephERPFormat = false;
      _alephERPFormatString = "";
    }
    if(le->_rbStringFormat->isChecked())
    {
      ...
      else if ( le->_rbBuiltinFormat->isChecked() )
      {
...
// dpinelo: -----------------------------------------
void ORGraphicsFieldItem::setAlephERPFormat(const QString &str)
{
    if (_alephERPFormat || !str.isEmpty() )
    {
        _alephERPFormat = true;
        _alephERPFormatString = str;
        _setModified(scene(), true);
    }
}
// dpinelo: -----------------------------------------
...
void ORGraphicsTextItem::properties(QWidget * parent)
{
  ...
  if(le->exec() == QDialog::Accepted)
  {
    ...
    // dpinelo --------------------------------------------------------------------------
    if(le->tbAlephERPFormat->text().isEmpty())
    {
        _alephERPFormat = false;
        _alephERPFormatString = "";
    }
    ...
    {
        setAlephERPFormat(le->tbAlephERPFormat->text());
    }
    // dpinelo --------------------------------------------------------------------------
...
void ORGraphicsTextItem::setAlephERPFormat(const QString &str)
{
    if ( !str.isEmpty() )
    {
        _alephERPFormat = true;
...
void ORGraphicsImageItem::setInlineImageData(QString dat)
{
  if(img_data != dat)
  {
    img_data = dat;
    // dpinelo: ---------
    QByteArray bytes = QByteArray::fromBase64(img_data.toUtf8());
    // dpinelo: ---------
