    ...
    : QDialog(parent, fl)
{
    ...
    connect(tbAlephERPFormat, SIGNAL(textChanged(const QString&)), this, SLOT(tbText_textChanged(const QString&)));
...
void TextEditor::rbAlign_changed()
{
    ...
    Qt::Alignment f = 0;
    if(rbHAlignLeft->isChecked())
    {
        f |= Qt::AlignLeft;
    }
    if(rbHAlignCenter->isChecked())
    {
        f |= Qt::AlignHCenter;
    }
    if(rbHAlignRight->isChecked())
    {
        f |= Qt::AlignRight;
    }
    if(rbVAlignTop->isChecked())
    {
        f |= Qt::AlignTop;
    }
    if(rbVAlignMiddle->isChecked())
    {
        f |= Qt::AlignVCenter;
    }
    if(rbVAlignBottom->isChecked())
    {
        f |= Qt::AlignBottom;
    }
    // dpinelo
    if(rbHAlignJustify->isChecked())
    {
        f |= Qt::AlignJustify;
    }
