...
static void notifyErrorListeners(XSqlQuery * source)
{
  ...
  // dpinelo:
  qDebug() << "OpenRPTPlugn::notifyErrorListeners: " << source->executedQuery();
  qDebug() << "OpenRPTPlugn::notifyErrorListeners: " << source->lastError().text();
}
