...
bool parseReportLabel(const QDomElement & elemSource, ORLabelData & labelTarget)
{
  ...
  for (int paramCounter = 0; paramCounter < params.count(); paramCounter++)
  {
    ...
    qDebug() << "parseReportLabel: Analizando: [" << elemParam.tagName() << "]";
    ...
    // dpinelo: --------------------------------------
    else if (elemParam.tagName() == "justify")
      labelTarget.align |= Qt::AlignJustify;
    else if (elemParam.tagName() == "wordwrap")
      labelTarget.align |= Qt::TextWordWrap;
    // dpinelo: --------------------------------------
...
bool parseReportField(const QDomElement & elemSource, ORFieldData & fieldTarget)
{
  ...
  fieldTarget.builtinFormat = false;
    fieldTarget.alephERPFormat = false;
  ...
  for (int paramCounter = 0; paramCounter < params.count(); paramCounter++)
  {
    ...
    {
        fieldTarget.align |= Qt::AlignJustify;
    }
    // dpinelo: --------------------------------------
    else if (elemParam.tagName() == "vcenter")
    ...
    // dpinelo ---------------------------------------------------
    else if (elemParam.tagName() == "alephERPFormat")
    {
        fieldTarget.alephERPFormat = true;
        fieldTarget.alephERPFormatString = elemParam.text();
    }
    // dpinelo ---------------------------------------------------
...
bool parseReportText(const QDomElement & elemSource, ORTextData & textTarget)
{
  ...
  textTarget.bottompad = 0;
    textTarget.alephERPFormat = false;
  ...
  for (int paramCounter = 0; paramCounter < params.count(); paramCounter++)
  {
    ...
    {
        textTarget.align |= Qt::AlignJustify;
    }
    else if (elemParam.tagName() == "alephERPFormat")
    {
        textTarget.alephERPFormat = true;
        textTarget.alephERPFormatString = elemParam.text();
    }
    // dpinelo: --------------------------------------
    else if (elemParam.tagName() == "vcenter")
...
bool parseReportImage(const QDomElement & elemSource, ORImageData & imageTarget)
{
  ...
  for(int paramCounter = 0; paramCounter < params.count(); paramCounter++)
  {
    ...
    {
      // ok we have an __inline image here
...
// dpinelo: -----------------------
qreal ORFieldData::calculateHeightWithWordWrap(const QString &textToShow)
{
    ...
    if ( alephERPFormat)
    {
        QStringList parts = alephERPFormatString.split(".");
        if ( parts.size() == 2 )
        {
