...
struct QParenthesis
{
    ...
    __inline QParenthesis()
        : id(0), role(0), offset(0), length(0)
    {}

    __inline QParenthesis(int i, quint8 r, int pos, int len)
...
public:
    ...
    __inline bool operator == (const QDocumentLineHandle* h) const
    {
        return m_handle == h;
    }
    ...
    __inline QString indentation() const
    {
        int idx = firstChar();
    ...
    __inline QDocumentLineHandle* handle() const
    {
        return m_handle;
    }
