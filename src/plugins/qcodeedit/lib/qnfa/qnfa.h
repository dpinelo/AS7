...
struct QCharTreeNode
{
    __inline QCharTreeNode(quint16 v = 0)
    {
        value.unicode = v;
    }
    __inline QCharTreeNode(const QCharTreeNode& o)
    {
        value = o.value;
        next = o.next;
    }
...
{
    __inline QNFAMatchContext(QNFA *root = 0) : context(root) {}

    __inline QNFAMatchContext& operator = (QNFAMatchContext *c)
    {
        if ( c )
        {
            context = c->context;
    ...
    __inline QNFAMatchContext& operator = (const QNFAMatchContext& c)
    {
        context = c.context;
        parents = c.parents;
        meaningless = c.meaningless;
...
private:
    struct Command
    {
        __inline Command(int p, int len, int act)
...
public:
    __inline QNFAMatchNotifier(QNFAMatchHandler *h)
        : handler(h) {}

    __inline QNFAMatchNotifier& operator = (const QNFAMatchNotifier& n)
    {
        handler = n.handler;
    ...
    __inline void operator () (int pos, int len, int action)
    {
    ...
    __inline int bufferLevel() const
    {
        return m_buffers.count();
    }

    __inline void startBuffering()
    {
        m_buffers.push(CommandList());
    }

    __inline void stopBuffering()
    {
