...
struct QLineMark
{
    __inline QLineMark() : line(-1) {}

    __inline QLineMark(const QString& f, int l, int m)
        : mark(m), line(l), file(f)
    {}

    __inline bool operator == (const QLineMark& m)
    {
...
{
    __inline QLineMarkHandle() : line(0) {}

    __inline QLineMarkHandle(const QString& f, QDocumentLineHandle *l, int m)
        : mark(m), line(l), file(f)
    {}

    __inline bool operator == (const QLineMarkHandle& m)
    {
        return (line == m.line) && (file == m.file) && (mark == m.mark);
    }
